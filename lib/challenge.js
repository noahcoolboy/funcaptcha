...
class Challenge {
    ...
    getHeaders() {
        const requestedId = crypt_1.default.encrypt(JSON.stringify({ sc: [17 + Math.ceil(Math.random() * 268), 198 + Math.ceil(Math.random() * 30)] }), `REQUESTED${this.data.session_token}ID`);
        const { cookie: tCookie, value: tValue } = util_1.default.getTimestamp();
...
class Challenge4 extends Challenge {
    ...
    answer(tile) {
        return __awaiter(this, void 0, void 0, function* () {
            (0, console_1.assert)(tile >= 0, "Tile must be >= 0");
            const apiBreaker = this.data.game_data.customGUI.api_breaker;
            // @ts-ignore
            if (typeof (apiBreaker) === "object" && apiBreaker.key && apiBreaker.value) {
