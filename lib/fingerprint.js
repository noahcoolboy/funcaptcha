...
function getFingerprint() {
    var fingerprint = __assign({}, baseFingerprint); // Create a copy of the base fingerprint
    // Randomization time!
    ...
    fingerprint["JSF"] = fingerprint["JSF"].filter(function (x) { return Math.random() > 0.5; });
    fingerprint["P"] = fingerprint["P"].filter(function (x) { return Math.random() > 0.5; });
    ...
    for (var i = 0; i < keys.length; i++) {
        switch (keys[i]) {
            case "CFP":
                ...
                break;
            case "P":
                fe.push(keys[i] + ":" + fingerprint[keys[i]].map(function (v) { return v.split("::")[0]; }));
                break;
            default:
