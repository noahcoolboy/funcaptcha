...
let apiBreakers2 = {
    ...
    type_4: {
        ...
        key: {
            ...
            epsilon: function (c) {
                for (var arr = [], r1 = Math.round(5 * Math.random()) + 1, r2 = Math.floor(Math.random() * r1), i = 0; i < r1; i++) {
...
function breakerValue(values, breakers) {
    return values.reduce((currentFunc, value) => {
        return breakers[value] ? (answer) => currentFunc(breakers[value](answer)) : currentFunc;
    }, (func) => func);
}
...
function getBda(userAgent, surl, referer, location, canvasFp) {
    var _a;
