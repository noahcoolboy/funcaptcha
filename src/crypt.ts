...
function encrypt(data: string, key: string): string {
    ...
    let aes = createCipheriv("aes-256-cbc", Buffer.from(salted, "hex").slice(0, 32), Buffer.from(salted, "hex").slice(32, 32 + 16))
...
function decrypt(rawData: string, key: string): string {
    ...
    let aes = createDecipheriv("aes-256-cbc", Buffer.from(result, "hex").slice(0, 32), Buffer.from(data.iv, "hex"))
